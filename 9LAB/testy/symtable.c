...
void PrintSym(struct SymbTab *s)
{
	...
	// 	break;
	default:
		string = "Unknown subtype in print symbol";
	}
	switch (s->Type) {
	case A_Decaf_INT:
		display_type = "INT ";
	...
	case A_Decaf_VOID:
		...
		break;
	default:
		string = "Unknown Declared TYPE in print symbol";
	}
	// changed this to use the c space formatting so it always looks pretty.
...
int Delete(int level)
{
	...
	while (p != NULL) {
		/* do we match? */
		if (p->level >=
		    level) { /* if it is the first in the list we have to update first, we know this by f being NULL */
			SIZE += p->mysize;
			if (f == NULL)
				first = p->next;
