...
main:   # we need 3 words to store the value, and then RA and SP
        ...
        # we only need 3 values. RA, SP, and the value we are reading
        subu $a0, $sp, 12 
        sw $sp, ($a0) # store the SP at the 0
        sw $ra, 4($a0)  # store the RA at the 4 offset
        move $sp, $a0  # adjust the stack pointer to the new value

	#Prints the prompt2 string
	li $v0, 4 # for syscall to print
	la $a0, L1 # L1 is "Please enter value"
	syscall 

	#reads one integer from user and saves in t0
	li $v0, 5 # set the syscall to read input
	#addu $t0, $sp, 8   #$t0 is the memory location for our variable
	syscall
	sw $v0 8($sp) # store the integer value we got in offset 8
	
	# print it back out  string
	li $v0, 4
	la $a0, L2
	syscall 
